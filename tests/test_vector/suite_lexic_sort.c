...
void test_norm(void)
{
    ...
    TEST_ASSERT_EQUAL_UINT8_MESSAGE(0, vector_lexic_sort(&vec, 9, 63), "a");

    for(int32_t i = 0; i < 8; i++)
    {
        vector_get(&vec, i, &t1);
        vector_get(&vec, i + 1, &t2);
        TEST_ASSERT_TRUE_MESSAGE(t1 <= t2, "b");
    }
...
void test_random(void)
{
    ...
    for(int32_t k = 0; k < 100; k++){
        ...
        TEST_ASSERT_EQUAL_UINT8_MESSAGE(0, vector_lexic_sort(&vec, n, 63), "c");
...
void test_string(void)
{
    ...
    TEST_ASSERT_EQUAL_UINT8_MESSAGE(0, vector_lexic_sort(&vec, 9, 4), "e");
...
void test_random_string(void)
{
    ...
    for(int32_t k = 0; k < 100; k++)
    {
        ...
        TEST_ASSERT_EQUAL_UINT8_MESSAGE(0, vector_lexic_sort(&vec, n, len), "k");
